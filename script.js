...
const saveImage = () => {
  ...
  ctx.filter = `brightness(${brightness}%) saturate(${saturation}%) invert(${inversion}%) grayscale(${grayscale}%) blur(${Blur}%) opacity(${opacity}%) contrast(${Contrast}%) shadows(${shadows}%) hue(${hue}deg) sepia(${sepia}%)`;
